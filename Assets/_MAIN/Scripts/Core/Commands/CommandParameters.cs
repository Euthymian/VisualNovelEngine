...
namespace COMMAND
{
    public class CommandParameters
    {
        ...
        public CommandParameters(string[] parameterArray, int startingIndex = 0)
        {
            ...
            {
                if (parameterArray[i].StartsWith(PARAMETER_IDENTIFIER) && !float.TryParse(parameterArray[i], out _)) // _ means discard = dont save the value as temporary variable
                {
        ...
        public bool TryGetValue<T>(string parameterNames, out T value, T defaultValue = default(T)) => TryGetValue(new string[] { parameterNames }, out value, defaultValue);
