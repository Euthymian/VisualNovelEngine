...
namespace DIALOGUE
{
    public class DL_COMMAND_DATA
    {
        ...
        private string[] RipArgs(string argsRawArray)
        {
            ...
            */
            ...
            for (int i = 0; i < argsRawArray.Length; i++)
            {
                if (argsRawArray[i] == '"')
                {
                    inQuote = !inQuote;
                ...
                if (!inQuote && argsRawArray[i] == ARGUMENTS_SEPARATOR)
                {
                ...
                currentArg.Append(argsRawArray[i]);
            }
